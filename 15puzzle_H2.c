...
/*Heuristic-2*/
int mismatchcount(int X[][4],int Y[][4])
{
	int count=0;
	int i,j,m,n,k;
	for(i=0;i<=3;i++)
	...
	while (check(openQueue)==1){
		...
		for(i=0;i<4;i++)
		{
			...
			if(boundaryCheck(closeNode->x_cor+row[i],closeNode->y_cor+col[i])==1){
				 ...
				 int heur_2 = mismatchcount(childNode->A,goal);
				 childNode->pathCost =childNode->level + heur_2; 
				 space += 1;
